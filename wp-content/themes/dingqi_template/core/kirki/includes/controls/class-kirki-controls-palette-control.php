...
if ( ! class_exists( 'Kirki_Controls_Palette_Control' ) ) {
	class Kirki_Controls_Palette_Control extends Kirki_Customize_Control {
		...
		public function enqueue() {
			...
			Kirki_Styles_Customizer::enqueue_customizer_control_script( 'kirki-palette', 'controls/palette', array(
				'jquery',
				'jquery-ui-button'
			) );
		}
					...
					<# } #>
						...
						<div id="input_{{ data.id }}" class="buttonset">
							<# for ( key in data.choices ) { #>
								<input type="radio" value="{{ key }}" name="_customize-palette-{{ data.id }}"
