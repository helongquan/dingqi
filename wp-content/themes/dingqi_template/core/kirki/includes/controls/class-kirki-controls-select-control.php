...
if ( ! class_exists( 'Kirki_Controls_Select_Control' ) ) {
	class Kirki_Controls_Select_Control extends Kirki_Customize_Control {
		...
		public function enqueue() {
			...
			Kirki_Styles_Customizer::enqueue_customizer_control_script( 'kirki-select', 'controls/select', array(
				'jquery',
				'selectize'
			) );
		}
							...
							<# if ( data.label ) { #>
								...
								<# } #>
									<# if ( data.description ) { #>
										...
										<# } #>
											<select {{{ data.link }}} data-multiple="{{ data.multiple }}"
											<# if ( 1
											<
											...
											<# } #>>
												...
												<# for ( key in data.value ) { #>
													<option value="{{ data.value[ key ] }}" selected>{{ data.choices[
														data.value[ key ] ] }}
													</option>
													<# } #>
														<# for ( key in data.choices ) { #>
															<# if ( data.value[ key ] in data.value ) { #>
																<# } else { #>
																	<option value="{{ key }}">{{ data.choices[ key ]
																		}}
																	</option>
																	}
																	<# } #>
