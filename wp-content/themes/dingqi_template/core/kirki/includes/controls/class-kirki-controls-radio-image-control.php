...
if ( ! class_exists( 'Kirki_Controls_Radio_Image_Control' ) ) {
	class Kirki_Controls_Radio_Image_Control extends Kirki_Customize_Control {
		...
		public function enqueue() {
			Kirki_Styles_Customizer::enqueue_customizer_control_script( 'kirki-radio-image', 'controls/radio-image', array(
				'jquery',
				'jquery-ui-button'
			) );
		}
				...
				<# } #>
					...
					<div id="input_{{ data.id }}" class="image">
						<# for ( key in data.choices ) { #>
							<input class="image-select" type="radio" value="{{ key }}"
							       name="_customize-radio-{{ data.id }}" id="{{ data.id }}{{ key }}" {{{ data.link }}}
