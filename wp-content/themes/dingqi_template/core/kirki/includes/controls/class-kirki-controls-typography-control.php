...
if ( ! class_exists( 'Kirki_Controls_Typography_Control' ) ) {
	class Kirki_Controls_Typography_Control extends Kirki_Customize_Control {
		...
		public function enqueue() {
			...
			Kirki_Styles_Customizer::enqueue_customizer_control_script( 'kirki-typography', 'controls/typography', array(
				'jquery',
				'selectize'
			) );
		}
								...
								<# if ( data.choices['font-style']['bold'] ) { #>
									<div class="bold">
										<label for="bold_{{ data.id }}">
											<input name="bold_{{ data.id }}" id="bold_{{ data.id }}" type="checkbox"
											       value="{{ data.value['bold'] }}" {{{ data.link }}}
											<# if ( '1' == data.value['bold'] ) { #> checked
												<# } #>>
										...
										<# if ( data.choices['font-style']['italic'] ) { #>
											<div class="italic">
												<label for="italic_{{ data.id }}">
													<input name="italic_{{ data.id }}" id="italic_{{ data.id }}"
													       type="checkbox" value="{{ data.value['italic'] }}" {{{
													       data.link }}}
													<# if ( '1' == data.value['italic'] ) { #> checked
														<# } #>>
												...
												<# if ( data.choices['font-style']['underline'] ) { #>
													<div class="underline">
														<label for="underline_{{ data.id }}">
															<input name="underline_{{ data.id }}"
															       id="underline_{{ data.id }}" type="checkbox"
															       value="{{ data.value['underline'] }}" {{{ data.link
															       }}}
															<# if ( '1' == data.value['underline'] ) { #> checked
																<# } #>>
																	<span
																		class="dashicons dashicons-editor-underline"></span>
														</label>
													</div>
													<# } #>
														<# if ( data.choices['font-style']['strikethrough'] ) { #>
															<div class="strikethrough">
																<label for="strikethrough_{{ data.id }}">
																	<input name="strikethrough_{{ data.id }}"
																	       id="strikethrough_{{ data.id }}"
																	       type="checkbox"
																	       value="{{ data.value['strikethrough'] }}" {{{
																	       data.link }}}
																	<# if ( '1' == data.value['strikethrough'] ) { #>
																		checked
																		<# } #>>
																			<span
																				class="dashicons dashicons-editor-strikethrough"></span>
																</label>
															</div>
															<# } #>
							</div>
											...
											<div class="font-family">
												...
												<select class="font-family select2">
													<# for ( key in data.fonts ) { #>
														<option value="{{ key }}"
														<# if ( key === data.value['font-family'] ) { #> selected
															<# } #>>{{ data.fonts[ key ] }}</option>
																<# } #>
												</select>
											</div>
											<# } #>
												...
												<# if ( data.choices['font-size'] ) { #>
													<div class="font-size">
														...
														<input type="number" min="0" step="any"
														       value="{{ parseFloat( data.value['font-size'] ) }}"/>
														<select>
															<# if ( data.choices['units'] ) { #>
																<# for ( key in data.choices['units'] ) { #>
																	<option value="{{ data.choices['units'][ key ] }}"
																	<# if ( _.contains( data.value['font-size'], data.choices['units'][ key ] ) ) { #>
																		selected
																		<# } #>>{{ data.choices['units'][ key ]
																			}}</option>
																			<# } #>
																				<# } else { #>
														...
														<# if ( data.choices['font-weight'] ) { #>
															<div class="font-weight">
																...
																<select class="font-weight">
																	<option value="100"
																	<# if ( 100 == data.value['font-weight'] ) { #>
																		selected
																		<# } #>>100</option>
																			<option value="200"
																			<# if ( 200 == data.value['font-weight'] ) { #>
																				selected
																				<# } #>>200</option>
																					<option value="300"
																					<# if ( 300 == data.value['font-weight'] ) { #>
																						selected
																						<# } #>>300</option>
																							<option value="400"
																							<# if ( 400 == data.value['font-weight'] ) { #>
																								selected
																								<# } #>>400</option>
																									<option value="500"
																									<# if ( 500 == data.value['font-weight'] ) { #>
																										selected
																										<# } #>
																											>500</option>
																											<option
																												value="600"
																											<# if ( 600 == data.value['font-weight'] ) { #>
																												selected
																												<# } #>
																													>600</option>
																													<option
																														value="700"
																													<# if ( 700 == data.value['font-weight'] ) { #>
																														selected
																														<# } #>
																															>700</option>
																															<option
																																value="800"
																															<# if ( 800 == data.value['font-weight'] ) { #>
																																selected
																																<# } #>
																																	>800</option>
																																	<option
																																		value="900"
																																	<# if ( 900 == data.value['font-weight'] ) { #>
																																		selected
																																		<# } #>
																																			>900</option>
																</select>
															...
															<# } #>
																...
																<# if ( data.choices['line-height'] ) { #>
																	<div class="line-height">
																		...
																		<input type="number" min="0" step="any"
																		       value="{{ data.value['line-height'] }}"/>
																	</div>
																	<# } #>

																		<# if ( data.choices['letter-spacing'] ) { #>
																			<div class="letter-spacing">
																				<h5>{{ data.l10n['letter-spacing']
																					}}</h5>
																				<input type="number" min="0" step="any"
																				       value="{{ parseFloat( data.value['letter-spacing'] ) }}"/>
																				<select>
																					<# if ( data.choices['units'] ) { #>
																						<# for ( key in data.choices['units'] ) { #>
																							<option
																								value="{{ data.choices['units'][ key ] }}"
																							<# if ( _.contains( data.value['letter-spacing'], data.choices['units'][ key ] ) ) { #>
																								selected
																								<# } #>>{{
																									data.choices['units'][
																									key ] }}</option>
																				...
																				<# if ( data.choices['color'] ) { #>
																					<div class="color">
																						...
																						<input type="text"
																						       ...
																						       class="kirki-color-control color-picker"
																						       {{{ data.link }}}/>
																					</div>
																					<# } #>
					</div>
