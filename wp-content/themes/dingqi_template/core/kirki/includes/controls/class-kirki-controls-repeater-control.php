	...
	class Kirki_Controls_Repeater_Control extends Kirki_Customize_Control {
		...
		public function __construct( $manager, $id, $args = array() ) {
			...
			if ( empty( $this->button_label ) ) {
				$this->button_label = esc_attr__( 'Add new row', 'tm-finance' );
			}
		...
		public function enqueue() {
			Kirki_Styles_Customizer::enqueue_customizer_control_script( 'kirki-repeater', 'controls/repeater', array(
				'jquery',
				'customize-base'
			), true );
		...
		public function render_content() { ?>
			<?php if ( '' != $this->help ) : ?>
				<a href="#" class="tooltip hint--left" data-hint="<?php echo esc_html( $this->help ); ?>"><span
			...
			<label>
				...
				<?php endif; ?>
				<input type="hidden" <?php $this->input_attrs(); ?>
				       value="" <?php echo esc_url( $this->get_link() ); ?> />
			</label>
				...
				<# var field; var index = data['index']; #>
					...
					<li class="repeater-row" data-row="{{{ index }}}">
						...
						<div class="repeater-row-header">
							...
							<span class="repeater-row-minimize"><i
									class="dashicons dashicons-arrow-up repeater-minimize"></i></span>
							<span class="repeater-row-move"><i
									class="dashicons dashicons-sort repeater-move"></i></span>
						</div>
												...
												<# } else if ( field.type === 'checkbox' ) { #>
													...
													<# } else if ( field.type === 'select' ) { #>
														...
														<label>
															<# if ( field.label ) { #>
																<span
																	class="customize-control-title">{{ field.label }}</span>
																<# } #>
																	<# if ( field.description ) { #>
																		<span
																			class="description customize-control-description">{{ field.description }}</span>
																		<# } #>
																...
																<# if ( field.label ) { #>
																	...
																	<# } #>
																		<# if ( field.description ) { #>
																			...
																			<# } #>
																				<textarea rows="5"
																				          data-field="{{{ field.id }}}">{{ field.default }}</textarea>
																				...
																				<# } else if ( field.type === 'image' ) { console.log(field); #>
																					...
																					<div class="actions">
																						<button type="button"
																						        class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>">
																							{{ field.buttonLabels.remove
																							}}
																						</button>
																						<button type="button"
																						        class="button upload-button"
																						        data-label="{{{ field.buttonLabels.default }}}"
																						        data-alt-label="{{{ field.buttonLabels.change }}}">
																							<# if ( field.default ) { #>
																								{{
